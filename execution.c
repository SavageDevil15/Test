...
void terminate(PCB *jobPCB, INTERRUPTS *interrupts, FILE *fWritePtr, int errCount, ...){
    ...
    fprintf(fWritePtr, "%s", " ---------------------------------------------------------------\n");
    fprintf(fWritePtr, "|  %3d  |  %3d  |  %3d  |  %3d  |  %3d  |  %3d  |  %3d  |  %3d  |\n ", jobPCB->JID, jobPCB->TTC, jobPCB->TLC, jobPCB->TTL, jobPCB->TLL, interrupts->SI, interrupts->TI, interrupts->PI);
    fprintf(fWritePtr, "%s", "---------------------------------------------------------------\n");
...
// ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
        ...
        if((memContext->IR[0] == 'L') && (memContext->IR[1] == 'R')){
            ...
            if(isValidOperand){
                ...
                // * CHECKING FOR INVALID PAGE FAULT
                ...
                if(!isValid){
                    terminate(jobPCB, interrupts, memContext->fWritePtr, 1, INVALID_PAGE_FAULT_ERROR);
                    break;
                }

                memcpy(memContext->R, memContext->IR, 4);
            }
        }
// ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
        // * SR
...
// ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
        // * CR
            ...
            if(isValidOperand){
                ...
                // * CHECKING FOR INVALID PAGE FAULT ON ANY OF THE ADDRESSES              
                // ? Mapping IRStart and RStart with their real addresses
                int PTRBaseAddr = memContext->PTR * PAGE_SIZE;
                int PTE = PTRBaseAddr + (virtualIRStart / PAGE_SIZE);
                checkIfPageFault(memContext->M, PTE, &isValid);
                if(!isValid){
                    terminate(jobPCB, interrupts, memContext->fWritePtr, 1, INVALID_PAGE_FAULT_ERROR);
                    break;
                ...
                PTRBaseAddr = memContext->PTR * PAGE_SIZE;
                PTE = PTRBaseAddr + (virtualRStart / PAGE_SIZE);
                checkIfPageFault(memContext->M, PTE, &isValid);
                if(!isValid){
                    terminate(jobPCB, interrupts, memContext->fWritePtr, 1, INVALID_PAGE_FAULT_ERROR);
                    break;
...
// ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
        // * PD
            ...
            if(isValidOperand){
                ...
                // * CHECKING WHETHER THE LOCATION ACTUALLY EXISTS i.e INVALID PAGE FAULT?
                if(!isValid){
                    terminate(jobPCB, interrupts, memContext->fWritePtr, 1, INVALID_PAGE_FAULT_ERROR);

...
void createNewPage(MEMCONTEXT *memCntx, int offset, int *newPageOffset){
	*newPageOffset = allocate(memCntx->M, REAL_MEMORY_LEN);
	...
	memset(memCntx->M[*newPageOffset*10], '\0', 4*PAGE_SIZE);
...
void checkIfValidOperand(char *IR, int *validOperandFlag){
    *validOperandFlag = FALSE;
    ...
    else if((('0' <= IR[2])&&(IR[2] <= '9')) && (('0' <= IR[3])&&(IR[3] <= '9'))){
        *validOperandFlag = TRUE;
    }
}
...
void resetContents(MEMCONTEXT *memContext, PCB *pcb, INTERRUPTS *interrupts){
	memContext->C = FALSE;
	memContext->IC = 0;
	memContext->PTR = -1;
	memset(memContext->IR, '\0', 4);
